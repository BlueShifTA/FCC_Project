...
void Combine_HTBin(){
   ...
   // Collect histograms
   TFile *file = new TFile("Result_Zjet_1.root","RECREATE");
   ...
   for (Int_t i = 0 ; i < 16 ; i ++ ) {   // index for Selection Cut   
       ...
       for (Int_t i_main = 0 ;i_main<16;i_main++ ) {   // index for open each file
         ...
         factor = NormLumi*xseczjet[i_main]*1000.f/Entry[i_main]->GetEntries();//GetSumOfWeights();
         cout << "Number: " << i_main << " xsec: " << xseczjet[i_main] << " factor: " << factor << " Entry: " << Entry[i_main]->GetEntries() << endl;
